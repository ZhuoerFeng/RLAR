...
class MegatronModelMerger(BaseModelMerger):
    ...
    def merge_and_save(self):
        ...
        for key in list(merged_state_dict):
            name = self._replace_name(key, params_mapping)
            if name is None:
                print(f"Warning: No mapping found for key {key}, skipping.")
                continue
            print(f"Replacing {key} with {name}")
            merged_state_dict[name] = merged_state_dict[key] 
            merged_state_dict.pop(key, None)  # remove old key
